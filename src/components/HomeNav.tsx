...
const HomeNav = ({ text, link }: HomeNavProps) => {
  ...
  return (
    <motion.div variants={childVar}>
      <Link
        href={link}
        className="group px-3 py-6 bg-bg min-[500px]:text-sm text-xs text-textlight font-body font-medium cursor-pointer border border-bglight hover:border-bglighter rounded-md flex items-center justify-between"
      >
